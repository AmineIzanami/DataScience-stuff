...
def create_dataset(dataframe,portion=0.33):
    '''
...
def apply_my_model_real_life(model, dataframe):
    '''
...
def test_all_classfiers(dataframe):
    ...
    for item in classifiers:
        ...
        correlation = pd.np.corrcoef(pd.np.array(y_test[pr.PowerPV]),pred)
...
def forward_selection_features(data_model):
    ...
    for i in range(len(all_combi)):
        ...
        predicted_df = apply_my_model_real_life(model, part_to_predict)
        mse = mean_squared_error(predicted_df.observed, predicted_df.predicted)
...
def model_to_pmml():
    ...
    power_pipeline = PMMLPipeline([
        ("classifier",svm.SVR(C=2, epsilon=0))
    ])
...
check_timeindex(data_cleaned_hourly)
data_weather_weatherbit = prw.get_all_weather_weatherbit()
check_timeindex(data_weather_weatherbit)
data_weather_darksky = prw.get_all_weather_darksky()
check_timeindex(data_weather_darksky)
...
data_model[pr.PowerPV] = data_cleaned_hourly[pr.PowerPV].copy()
# data_model['API darksky apparentTemperature'] = (data_weather_darksky.loc[data_cleaned_hourly.index]['apparentTemperature']).copy()
...
data_model['API_darksky_visibility'] = data_weather_darksky.loc[data_cleaned_hourly.index]['visibility']
...
data_model_featured = data_model[[pr.PowerPV]+['API_darksky_humidity', 'hours']].copy()
